...
In the [post](https://adam-lund.github.io/Covid19-NP/) we looked at how many (Covid-19) tests one (Jon) would have to take to obtain a certain accuracy; Jon would need to test 9 times to obtain (more than)   99 $\%$ accuracy with a Covid-19 test that is 85 $\%$ accurate using the LRT based decision rule. To  answer this question we did a power analysis to obtain  the minimum number of tests we would need for our likelihood ratio test to attain a certain power.
