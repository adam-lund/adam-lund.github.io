...
In the special case with   $\alpha=\beta=1-a^\ast$, such that we  have  overall test accuracy $a^\ast$,
   by \eqref{eq6}   $b_1=1/b_0$ hence   $\vert \log(b_1)\vert=\vert \log(b_0)\vert$.    From \eqref{eq13} it follows that  we must have  $n(a):=n_0=n_1$ finally yielding    
