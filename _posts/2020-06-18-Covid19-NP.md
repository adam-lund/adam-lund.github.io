...
###### Neyman and Pearson
Now, what we should do,  is some how, given $m$ single trial covid-19  tests answers (data) $t_1,\ldots,t_m$ where $t_j\in0,1\\}$ with $0$ meaning covid-19 negative,  device an overall test  $D_m\in\\{0,1\\}$, that with probability $a^\ast$, gives ud the correct diagnosis. But what does this test look like? How do we combine $m$ tests  into one overall test $D_m$ in way that let us control the accuracy?  
